...
public class RotationControl extends CommandBase {
  ...
  */
  ...
  public void execute() {
    ...
    ColorSensor.getInstance().spinWheel(.3);
    count = ColorSensor.getInstance().getTicks();
    System.out.println("tick cnt" + ColorSensor.getInstance().getTicks());
  ...
  public boolean isFinished() {
    System.out.println("end");
      if(Math.abs( count ) > 4000 ){
        ColorSensor.getInstance().spinWheel(0);
        System.out.println("finisned");
        return true;
      }else{
      return false;
      }
